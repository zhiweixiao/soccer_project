...
England_poisson_model <- glm(home_team_goal ~ home_team_name + away_team_name, family = "poisson", data = England_data)
England_nb_model <- glm.nb(home_team_goal ~ home_team_name + away_team_name, data = England_data)
...
for (i in 4:29) {
  ...
  Belgium_mse_poisson[i] = mean((Belgium_poisson_model_prediction - subset(Belgium_data, stage == i+1)$home_team_goal)^2)
}
Belgium_mse_poisson # better
...
for (i in 4:29) {
  ...
  Belgium_mse_nb[i] = mean((Belgium_nb_model_prediction - subset(Belgium_data, stage == i+1)$home_team_goal)^2)
}
Belgium_mse_nb

sum(Belgium_mse_poisson > Belgium_mse_nb, na.rm =TRUE)
